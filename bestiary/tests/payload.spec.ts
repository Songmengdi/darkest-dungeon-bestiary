/* lib/payload.ts 的数据契约测试:锁死 .darkest 原始记录 → 图鉴 JSON 形状的翻译语义。
 * 前端 src/types.ts 是这些形状的手工镜像 —— 这里红了,先改 payload,再对齐 types。 */
import { describe, expect, it } from "vitest";
import {
  asArray, brainPayload, lootPayload, prettyId, rankDigits, skillPayload, tierPayload, type NameLookup,
} from "../lib/payload.ts";
import type { TierData } from "../lib/dataIndex.ts";
...
describe("skillPayload —— target 串语义(build 期判定,UI 只消费布尔)", () => {
